...
CUTE_HOST_DEVICE
void
invalidate_barrier(uint64_t& smem_barrier)                 // 64 bits user-manged barrier in smem
{
...
CUTE_HOST_DEVICE
void
arrive_barrier_cluster(uint64_t& smem_barrier,             // 64 bits user-manged barrier in smem
                       uint32_t remote_cta_id)
{
  ...
  asm volatile(
    ...
    "}\n"
    :: "r"(smem_int_ptr), "r"(remote_cta_id));
#endif
}
...
CUTE_DEVICE
void
warpgroup_sync(uint32_t num_warpgroups,
               uint32_t barrier_id)
{
...
warp_specialization(__grid_constant__ TMA_A const tma_A,
...
torch::Tensor
run_warp_specialization(const torch::Tensor a, const torch::Tensor b, std::optional<torch::Tensor> _c) {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("warp_specialization_fp16_fp16_fp16_fp16", &(run_warp_specialization<128, 256, 64, 3, cute::half_t, cute::half_t, cute::half_t>), "Run a mixed-precision half 16x8x8 MMA operation.");
  m.def("warp_specialization_fp16_fp16_fp16_fp32", &(run_warp_specialization<128, 256, 64, 3, cute::half_t, cute::half_t, cute::half_t, float>), "Run a mixed-precision half 16x8x8 MMA operation.");
  m.def("warp_specialization_bf16_bf16_bf16_fp32", &(run_warp_specialization<128, 256, 64, 3, cute::bfloat16_t, cute::bfloat16_t, cute::bfloat16_t, float>), "Run a mixed-precision half 16x8x8 MMA operation.");
}
