"""Block-level MMA (128x128x64) implemented with CuTe DSL (Python).

CuTe DSL counterpart of ``block_mma.cu`` / ``block_mma.py``. Extends
``04-tiled-copy`` from a single 32x32x16 tile to a 128x128x64 thread-block
tile using the SM80 ``mma.m16n8k16.bf16.bf16.f32.f32`` atom:

  * atom shape (M, N, K)        = (16, 8, 16)
  * atom_layout_mnk             = (2, 4, 1)     -> 8 atoms per block
  * value expansion (M, N, K)   = (1, 1, 2)     -> 2 K instructions per thread
  * permutation_mnk (block tile)= (32, 32, 32)  -> resulting MMA tile

Inside the block the MMA tile is replicated to cover the full 128x128x64
problem via ``tCrA / tCrB / tCrC`` having shape ``(MMA, MMA_M, MMA_N or
MMA_K)`` with MMA_M = MMA_N = 128/32 = 4 and MMA_K = 64/32 = 2.
``cute.gemm(tiled_mma, tCrC, tCrA, tCrB, tCrC)`` walks the full inner
M*N*K loop in registers automatically.

Output is narrowed bf16 from an fp32 accumulator.

Run with ``python cutedsl_block_mma.py``.
"""
...
# Per-MMA-tile size = atom * thr_layout * val_expand.
MMA_TILE_MNK = (
...
def block_mma_kernel(
    ...
    gO = cute.local_tile(mO, tiler=(M, N), coord=(0, 0))  # (M, N)

    thr_mma = tiled_mma.get_slice(tid)
    tCgC = thr_mma.partition_C(gC)
    tCrA = tiled_mma.make_fragment_A(thr_mma.partition_A(gA))  # (MMA, MMA_M, MMA_K)
    tCrB = tiled_mma.make_fragment_B(thr_mma.partition_B(gB))  # (MMA, MMA_N, MMA_K)
    ...
    # Single ``cute.gemm`` over the full (MMA_M, MMA_N, MMA_K) fragment tile.
...
def block_mma_gemm(
    ...
    # SM80 ``mma.m16n8k16.bf16.bf16.f32.f32``.
...
def main() -> None:
    ...
    print("Compiling CuTe DSL block_mma_gemm kernels ...")
