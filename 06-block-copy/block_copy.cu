...
__global__ void
block_copy(void* __restrict__ Cptr,
...
struct KernelSpec {
  ...
  using SmemLayoutO = decltype(make_layout(make_shape(Int<kTileM>{}, Int<kTileN>{}),
                                           make_stride(Int<kTileN>{}, Int<1>{})));
...
torch::Tensor
run_block_copy(const torch::Tensor a,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("block_copy_bf16_bf16_bf16_fp32", &(run_block_copy<128, 128, 64, cute::bfloat16_t, cute::bfloat16_t, cute::bfloat16_t, float>), "Run a mixed-precision half 16x8x8 MMA operation.");
}
